...
import { OPENROUTER_GEMINI_25_FLASH } from "./types";
...
export async function regenerateAgentBundle(bundle: AgentBundle, instruction: string): Promise<GenerateBundleResult> {
  ...
  const regenerated: AgentBundle = { ...current, first_response: cleanText(llmResult?.first_response, current.first_response), sections: current.sections.map((section) => ({ ...section, prompt: overrides[section.section_key] || `${section.prompt}${toneRule}` })) };
